    ...
    <section ref={section} id="capabilities" className="section-pad relative border-y border-white/[.06] bg-[#0a0b0d]">
      <div className="site-grid">
        <div className="grid gap-20 lg:grid-cols-[.82fr_1.18fr]">
          ...
          <div className="relative lg:max-h-[calc(100vh-10rem)] lg:overflow-y-auto lg:pr-4 [scrollbar-color:rgba(125,223,255,.35)_transparent] [scrollbar-width:thin]">
            <div aria-hidden="true" className="absolute bottom-6 left-[25px] top-6 w-px bg-gradient-to-b from-[#3157ff]/65 via-[#9747ff]/25 to-transparent sm:left-[31px]" />
            {capabilities.map(([number, title, text], index) => (
              <motion.article
                ...
                className="group relative grid min-h-[132px] grid-cols-[52px_1fr] gap-5 border-b border-white/[.08] py-6 first:pt-0 sm:grid-cols-[64px_1fr]"
              >
                <div className="relative z-10 grid h-[52px] w-[52px] place-items-center rounded-full border border-white/10 bg-[#0a0b0d] font-mono text-[10px] text-[#7ddfff] transition-all duration-500 group-hover:border-[#4b72ff]/60 group-hover:bg-[#111529] sm:h-16 sm:w-16">
                  {number}
                </div>
                <div>
                  <div className="flex items-start justify-between gap-5">
                    <h3 className="max-w-xl text-[clamp(1.25rem,2.35vw,2.05rem)] font-medium leading-[1.08] tracking-[-.04em]">{title}</h3>
                  ...
                  <p className="mt-3 max-w-xl text-sm leading-6 text-white/42 xl:text-[15px]">{text}</p>
                  <motion.div
                    aria-hidden="true"
                    initial={{ scaleX: 0 }}
                    ...
                    className="mt-5 h-px origin-left bg-gradient-to-r from-[#3157ff]/55 via-[#9747ff]/15 to-transparent"
                  />
                </div>
              </motion.article>
            ))}
